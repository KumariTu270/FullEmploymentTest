...
object CSVReader {
      def main(args: Array[String]): Unit = {
            ...
            val areasAffected = affectedAreas.withColumnRenamed("State Name", "State_Name")
              ...
              .withColumn("District_Name", lower(col("District_Name")))



        val cleanedDistrictCode = districtCode
              ...
              .withColumn("Name of the State/Union territory and Districts", lower(col("Name of the State/Union territory and Districts")))


            val indiaAffectedWaterArea = areasAffected
              .join(cleanedDistrictCode , cleanedDistrictCode.col("Name of the State/Union territory and Districts") === areasAffected.col("District_Name"), "inner")
              .withColumn("Year", year(to_date($"Year", "dd/MM/yy").cast(DataTypes.DateType)))
              .drop("Name of the State/Union territory and Districts")

            indiaAffectedWaterArea.show(20)
            ...
            //Second Problem
            val totalCount = indiaAffectedWaterArea.groupBy("Quality_Parameter" , "Village_Name" , "Year")
                .count().as("Frequency")
